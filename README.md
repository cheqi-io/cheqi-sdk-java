...
Version `2.4.0` describes the API on this branch but has not been published to Maven Central yet. Until it is released, build the SDK locally or use the latest published release where its API is sufficient.

After `2.4.0` is published, add it with Maven:
...
```gradle
implementation 'io.cheqi:cheqi-sdk:2.4.0'
```
